...
export const UserController = {
    ...
    updateEmployee: async (req, res, next) => {
        try {
            const updatedEmployee = await UserService.updateEmployee(req.params.id, req.body);
            return res.json(successResponse({ data: updatedEmployee, message: "Employee updated" }));
        } catch (error) {
            next(error);
        }
    },
